...
// Once you have completed part 1 (decoding instructions), uncomment the below block
  ...
  unsigned int* registers = (unsigned int*)calloc(NUM_REGS , sizeof(unsigned int));
  ...
  unsigned char* memory = (unsigned char*)calloc(STACK_SIZE,sizeof(unsigned char));
...
unsigned int execute_instruction(unsigned int program_counter, instruction_t* instructions, unsigned int* registers, unsigned char* memory)
{
  ...
  case movl_imm_reg:
    ...
    break;
  case cmpl: ;
    int second = (int)registers[instr.second_register];
    int first = (int)registers[instr.first_register];
    long result = (long)second - (long)first;
    registers[0] = 0;
    if((result > INT32_MAX || result < INT32_MIN)){//signed overflow 
      registers[0] = registers[0] | (1 << 10);
    }
    if(result < 0){//unsigned overflow
      registers[0] = registers[0] | 1;
    }
    if((result >> 63) == 1){//signed flag set
      registers[0] = registers[0] | (1 <<6);
    }
    if(result == 0){
      registers[0] = registers[0] | (1 << 5);
    }
    break;
  case je:
  ...
  case call:
    ...
    memory[registers[8]] = program_counter;
    return program_counter + instr.immediate + 4;
    break;
  case ret:
    if(registers[8] == 1024){
  ...
  case pushl:
    registers[8] = registers[8] - 4;
    memcpy(&memory[registers[8]], &registers[instr.first_register], sizeof(int));
    break;
  case popl:
    memcpy(&registers[instr.first_register],&memory[registers[8]], sizeof(int));
    registers[8] = registers[8] + 4;
    break;
   case printr:
